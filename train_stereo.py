...
def sequence_loss(flow_preds, flow_gt, valid, loss_gamma=0.9, max_flow=700):
    ...
    metrics = {
        'epe': epe.mean().item(),
        '0.5px': torch.sum(epe > 0.5).item() / len(epe),
        '1px': torch.sum(epe > 1).item() / len(epe),
        '2px': torch.sum(epe > 2).item() / len(epe),
        '4px': torch.sum(epe > 4).item() / len(epe),
    }

    # flow_loss += torch.sum((left_shift - image2) ** 2, dim=1).sqrt()
...
def train(args):
    ...
    #
    a = torch.empty(2, 3, 320, 720)
    logger.writer.add_graph(model, (torch.empty(2, 3, 320, 720), torch.empty(2, 3, 320, 720)))
    ...
    for epoch in range(1, args.epoch+1):
        logging.info(f'This is {epoch}-th / {args.epoch} epoch')
        for i_batch, (_, *data_blob) in enumerate(tqdm(train_loader)):
            ...
            logger.push(metrics)
            if (i_batch+1) % 500 == 0:
                save_path = Path('checkpoints/epoch%d_%s_%d.pth' % (epoch, args.name, i_batch))
        ...
        save_path = Path('checkpoints/epoch%d_%s.pth' % (epoch, args.name))
        logging.info(f"Saving file {save_path.absolute()}")
        torch.save(model.state_dict(), save_path)
